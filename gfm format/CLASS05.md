...
- [Version 1 plot gdpPercap vs. LifeExp for all
  rows](#version-1-plot-gdppercap-vs-lifeexp-for-all-rows)
...
`install.packages("ggplot2")' > **N.B.** We never run an`install.packages()\`
in a code chunk that otherwise we will re-install needlessly every time
we render our document
...
I want to see a plot for each continent – in ggplot language this is
called “faceting”
